...
class Primer(Sequence):
    ...
    name: Optional[str] = Field(
        ...
        json_schema_extra={
            "linkml_meta": {
                "alias": "name",
                "domain_of": ["Primer", "CollectionOption", "CollectionOptionInfo", "Recombinase"],
...
class Recombinase(ConfiguredBaseModel):
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({"from_schema": "https://opencloning.github.io/OpenCloning_LinkML"})
    ...
    def pattern_site1(cls, v):
        pattern = re.compile(r"^[A-Z]+[a-z]+[A-Z]+$")
        if isinstance(v, list):
            for element in v:
                if isinstance(element, str) and not pattern.match(element):
                    err_msg = f"Invalid site1 format: {element}"
                    raise ValueError(err_msg)
        elif isinstance(v, str) and not pattern.match(v):
            err_msg = f"Invalid site1 format: {v}"
            raise ValueError(err_msg)
        return v

    @field_validator("site2")
    def pattern_site2(cls, v):
        pattern = re.compile(r"^[A-Z]+[a-z]+[A-Z]+$")
        if isinstance(v, list):
            for element in v:
                if isinstance(element, str) and not pattern.match(element):
                    err_msg = f"Invalid site2 format: {element}"
                    raise ValueError(err_msg)
        elif isinstance(v, str) and not pattern.match(v):
            err_msg = f"Invalid site2 format: {v}"
            raise ValueError(err_msg)
        return v
...
class RecombinaseSource(AssemblySource):
    ...
    type: Literal["RecombinaseSource"] = Field(
        default="RecombinaseSource",
        description="""Designates the class""",
        json_schema_extra={
            "linkml_meta": {
                "alias": "type",
...
Recombinase.model_rebuild()
RecombinaseSource.model_rebuild()
